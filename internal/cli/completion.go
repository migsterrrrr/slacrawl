...
_slacrawl()
{
	...
	case "${prev}" in
		--format)
			...
			;;
		--source)
			COMPREPLY=( $(compgen -W "api bot user desktop wiretap mcp connector all" -- "${cur}") )
			return
			;;
		--kind)
    ...
    args)
      case $words[2] in
        ...
        update)
          ...
          ;;
        sync)
          _arguments '--source[sync source]:source:(api bot user desktop wiretap mcp connector all)' '--workspace[workspace id]:workspace id:' '--channels[channel ids]:channels:' '--since[start timestamp]:timestamp:' '--full[run full sync]' '--latest-only[skip first-time historical backfills]' '--concurrency[worker count]:count:' '--with-media[fetch file media after sync]'
          ;;
        import)
